...
public class CornerSort {
    ...
    private static final NonDominatedSortingFactory INSTANCE = (maximumPoints, maximumDimension) ->
        ...
        private int findBestPoint(double[][] points, int from, int until, int objective) {
            int best = from;
            double[] bestPoint = points[indices[best]];
            for (int i = from + 1; i < until; ++i) {
                double[] currPoint = points[indices[i]];
                if (bestPoint[objective] > currPoint[objective] ||
                        bestPoint[objective] == currPoint[objective] && strictlyDominates(currPoint, bestPoint)) {
                    bestPoint = currPoint;
                    best = i;
                }
            }
        ...
        private int moveDominatedToTail(double[][] points, int index, int until) {
            double[] bestPoint = points[indices[index]];
            ++index;
            while (index < until) {
                if (strictlyDominates(bestPoint, points[indices[index]])) {
        ...
        private int peelCurrentRank(double[][] points, int[] ranks, int from, int until, int rank) {
            int curr = from;
            int last = until;
            int objective = 0;
            ...
            while (curr < last) {
                int best = findBestPoint(points, curr, last, objective);
                ArrayHelper.swap(indices, best, curr);
                ranks[indices[curr]] = rank;

                last = moveDominatedToTail(points, curr, last);
                curr = ++from;
