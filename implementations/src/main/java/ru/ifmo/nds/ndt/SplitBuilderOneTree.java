...
public class SplitBuilderOneTree {
    ...
    public SplitBuilderOneTree(int size) {
    ...
    private Split construct(int from, int until, int coordinate, int depth) {
        if (from + threshold < until) {
            int nextCoordinate = coordinate + 1 == maxCoordinate ? 1 : coordinate + 1; // TODO возможно можно начинать и с 1
    ...
    public Split result(double[][] transposedPoints, int nPoints, int dimension, int threshold) {
        ...
        Split result = construct(0, nPoints, 1, 0); // TODO возможно можно начинать и с 1
    ...
    public Split result(double[][] transposedPoints, int from, int until, int dimension, int threshold) {
        ...
        Split result = construct(from, until, 1, 0); // TODO возможно можно начинать и с 1
        this.transposedPoints = null;
        this.threshold = -1;
        return result;
    }
